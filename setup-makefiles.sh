...
LINEEND=" \\"
COUNT=`wc -l ../matissewifi/proprietary-files.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' ../matissewifi/proprietary-files.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' ../matissewifi/proprietary-files.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
